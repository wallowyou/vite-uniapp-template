...
const dataList = ref<{ id: number; title: string; content: '' }[]>([]) // 数据列表
...
const downCallback = (mescroll: any) => {
  apiNewList()
    .then((data) => {
      ...
      if (Array.isArray(data) && data.length > 0) dataList.value.unshift(data[0]) // 追加数据在前面
    })
    .catch(() => {
...
const upCallback = (mescroll: any) => {
  apiNewList(mescroll.num, mescroll.size)
    .then((curPageData: any) => {
